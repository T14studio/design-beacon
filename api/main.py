...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        # Security Hardening (Zero Regression)
        response.headers["Strict-Transport-Security"] = "max-age=31536000; includeSubDomains"
        response.headers["Content-Security-Policy"] = "default-src * 'unsafe-inline' 'unsafe-eval'; frame-ancestors 'none';"
        response.headers["Cross-Origin-Opener-Policy"] = "same-origin-allow-popups"
        response.headers["Cross-Origin-Resource-Policy"] = "cross-origin"
        
        # Ocultar headers de identificação
...
_HANDOFF_MESSAGE = (
    "Entendido! Vou encaminhar você para um especialista da nossa equipe com todo o contexto da conversa. "
    "Ele já vai saber do que se trata e entra em contato em breve. "
    "🤝 Fique à vontade!"
)
...
async def whatsapp_incoming_webhook(request: Request):
    ...
    # Removido print do FULL RAW PAYLOAD para evitar vazamento de PII (Hardening de Segurança)
    ...
    # DEBUG removido (Hardening) - Estrutura segura sem exposição
    ...
    try:
        ...
        reply         = ai_result.get("message_to_user", "Olha, tive um probleminha técnico aqui. Pode mandar sua mensagem de novo?")
        new_state     = ai_result.get("etapa_da_conversa", current_state)
        handed_off    = ai_result.get("handoff_recomendado", False)
        setor_destino = ai_result.get("setor_destino")
        prioridade    = ai_result.get("prioridade", "normal")
        ...
        if ai_result.get("acionar_busca_imoveis"):
            ...
            if props:
                ...
                reply = "Essas são as opções que achei para você. Alguma chamou atenção? Posso detalhar, agendar visita ou buscar outras se preferir 😊"
            else:
                reply = "Olha, não achei nada com exatamente esses filtros agora. Quer que eu tente com um valor um pouco diferente ou em outra região?"
        ...
        # ── 13. Envia resposta via WhatsApp — estratégia humanizada ─────────────
        # FILOSOFIA: Conversas reais no WhatsApp NÃO têm menus o tempo todo.
        # Botões APENAS em: (a) primeiro contato ou (b) decisão binária clara.
        # Turnos conversacionais (qualificacao, conducao) → só texto, sem menu.

        ctas = ai_result.get("sugestoes_de_cta") or []

        # CTAs que causam loop — nunca viram botão
        _LOOP_CTAS = {
            "mais opções", "receber opções", "ver opções", "mais detalhes",
            "comercial", "administração", "financeiro",  # setores já identificados
        }
        # Remove CTAs que causam loop ou são redundantes
        ctas_filtrados = [
            c for c in ctas
            if c.lower().strip() not in _LOOP_CTAS and len(c) <= 22
        ]

        botoes_rótulos: list = []

        # ABERTURA (primeiro contato) → 3 botões de setor, simples
        if new_state == "recepcao" or (is_first_turn and not setor_destino):
            botoes_rótulos = ["🏠 Imóveis", "📋 Administração", "💳 Financeiro"]

        # DECISÃO BINÁRIA CLARA → max 2 botões com ação concreta
        elif new_state in ("conducao", "encaminhamento") and setor_destino == "comercial":
            # Só botões de ação concreta: agendar/proposta
            acao_ctas = [c for c in ctas_filtrados if any(
                k in c.lower() for k in ["agendar", "visita", "proposta", "especialista", "simula"]
            )]
            botoes_rótulos = acao_ctas[:2]  # max 2

        elif new_state in ("conducao", "encaminhamento") and setor_destino in ("administrativo", "financeiro"):
            acao_ctas = [c for c in ctas_filtrados if any(
                k in c.lower() for k in ["agendar", "enviar", "segunda via", "contato", "falar"]
            )]
            botoes_rótulos = acao_ctas[:2]  # max 2

        # QUALIFICAÇÃO (coleta de dados) → SEM botões, conversa natural
        # else: botoes_rótulos permanece vazio → send_text puro

        send_result = None
        if botoes_rótulos:
            # Sempre envia como "button" (máx 3) — NUNCA "list" (evita "Ver opções")
            choices_formatted = []
            for lbl in botoes_rótulos:
                l_low = lbl.lower()
                label_clean = lbl.lstrip("🏠📋💳📄📅📜💵 ")
                if "imóve" in l_low or "imove" in l_low:
                    choices_formatted.append(f"🏡 Imóveis|IMOVEIS")
                elif "admin" in l_low:
                    choices_formatted.append(f"📄 Administração|ADMINISTRACAO")
                elif "finan" in l_low:
                    choices_formatted.append(f"💳 Financeiro|FINANCEIRO")
                elif "visit" in l_low:
                    choices_formatted.append(f"📅 Agendar visita|VISITA")
                elif "proposta" in l_low:
                    choices_formatted.append(f"✍️ Fazer proposta|PROPOSTA")
                elif "simula" in l_low:
                    choices_formatted.append(f"🔢 Simulação|SIMULACAO")
                elif "especialista" in l_low or "falar" in l_low:
                    choices_formatted.append(f"👤 Falar com especialista|ESPECIALISTA")
                else:
                    choices_formatted.append(f"{lbl}|{label_clean.upper().replace(' ', '_')[:20]}")

            btn_result = WhatsAppService.send_menu(
                telefone,
                reply,
                choices_formatted[:3],  # Hard cap: nunca mais que 3
                menu_type="button",     # SEMPRE button, nunca list
                footer="Axis • Ética"
            )
            ...
            if btn_result.get("ok"):
                ...
                # Fallback texto simples se Uazapi rejeitar botões
                send_result = WhatsAppService.send_text(telefone, reply)
        else:
            # Conversa pura — sem botões, como um consultor real no WhatsApp
            send_result = WhatsAppService.send_text(telefone, reply)
