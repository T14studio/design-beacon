...
class OpenAIService:
    ...
    async def process_turn(user_message: str, history: list, context: dict) -> dict:
        ...
        try:
            ...
            def _rb_detect_intent(msg: str) -> dict:
                m = (msg or "").lower()
                # Financeiro
                ...
                # Comercial
                if any(k in m for k in ["comprar", "compra", "vender", "anunciar", "avali", "visita", "agendar", "proposta", "financ", "simula", "alugar", "locaç", "locacao", "interesse"]):
            ...
            def _rb_build_reply(intent: dict) -> tuple[str, list[str], str, str]:
                ...
                if intent.get("setor") == "administrativo":
                    if intent.get("intencao") == "manutencao":
                        if intent.get("urgente"):
                            return (
                                f"{saud}Entendi que é URGENTE. Pode me dizer: há risco imediato (inundação, curto-circuito, choque) e qual o imóvel/contrato? Se puder, descreva em 1 frase o que está acontecendo agora.",
                                ["Descrever Urgência", "Acompanhar manutenção", "Falar com administrativo"],
                                "encaminhamento",
                                "manutencao_prioritaria",
                            )
                        return (
                            f"{saud}Certo. Para eu acionar o administrativo/manutenção: qual o imóvel/contrato e qual o problema? Se tiver fotos, descreva que você tem que eu oriento o próximo passo.",
                            ["Descrever problema", "Acompanhar manutenção", "Falar com administrativo"],
                            "qualificacao",
                            "administrativo",
                        )
                    return (
                        f"{saud}Entendi. Para eu encaminhar ao administrativo com precisão, me diga qual contrato/imóvel está relacionado e qual a demanda (documentação, assinatura, renovação ou rescisão).",
                        ["Enviar documentos", "Falar com administrativo", "Informar contrato"],
                        "qualificacao",
                        "administrativo",
                    )
                ...
                if intent.get("setor") == "comercial":
                    if intent.get("intencao") == "financiamento":
                        return (
                            f"{saud}{imovel_line}Ótimo — financiar é um caminho comum. Você prefere fazer uma simulação agora ou falar com um especialista para planejar as melhores condições?{(' Como é um imóvel só para ' + modo.lower() + ', já sigo por esse caminho.') if modo in ('Venda','Locação') else ''}",
                    ...
                    if intent.get("intencao") == "agendar_visita":
                        return (
                            f"{saud}{imovel_line}Perfeito. Qual dia e horário você prefere para a visita? Se quiser, me diga também seu telefone/WhatsApp para confirmação.",
                    ...
                    if intent.get("intencao") in ("comprar_imovel", "alugar_imovel", "imovel_especifico"):
                        ...
                        return (
                            f"{saud}{imovel_line}{suffix} Qual é seu objetivo agora: visitar, receber detalhes ou fazer proposta?",
                    ...
                    if intent.get("intencao") == "anunciar_imovel":
                        return (
                            f"{saud}Perfeito. Para começarmos a avaliação: qual o tipo do imóvel, bairro/endereço e o valor que você tem em mente? Se tiver fotos, isso ajuda bastante.",
                ...
                # Default
                return (
                    f"{saud}Entendi. Me conte em poucas palavras o que você precisa (compra/locação, contrato/documentos/manutenção, ou boletos/repasses).",
                    ["Comprar", "Locação", "Boletos", "Repasses", "Documentação"],
                    context.get("current_state", "recepcao"),
                    None,
                )

            intent = _rb_detect_intent(user_message)
            reply, ctas, etapa, setor = _rb_build_reply(intent)
