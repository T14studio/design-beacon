...
export default function AxisChat({ initialMessage, propertyContext, isOpen, onClose }: AxisChatProps) {
  ...
  // Simula atraso de digitação humana proporcional ao tamanho da resposta
  const simulateTypingDelay = (text: string): Promise<void> => {
    const charCount = (text || "").length;
    // ~50ms por caractere, entre 800ms e 3500ms
    const delay = Math.min(Math.max(charCount * 50, 800), 3500);
  ...
  const handleSendMessage = async (text: string) => {
    ...
    try {
      ...
      // ⏱️ Simula atraso de digitação humanizada antes de exibir a resposta
      await simulateTypingDelay(botMsgContent);
