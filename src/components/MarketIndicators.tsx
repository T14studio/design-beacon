    ...
    <div className="bg-card/40 backdrop-blur-xl border border-white/5 rounded-2xl sm:rounded-[2rem] md:rounded-[2.5rem] p-4 sm:p-6 md:p-8 lg:p-10 shadow-2xl overflow-hidden relative group">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4 md:gap-5">
        {indicators.map((ind, i) => (
          <div 
            key={ind.id} 
            className="bg-background/40 border border-white/5 rounded-xl sm:rounded-2xl md:rounded-3xl p-3.5 sm:p-4 md:p-5 transition-all duration-500 hover:scale-[1.03] hover:border-primary/20 hover:bg-background/60 group/card relative overflow-hidden"
            style={{ animationDelay: `${i * 100}ms` }}
            title={ind.description}
          >
            <div className="absolute top-0 right-0 p-2 sm:p-3 opacity-[0.03] scale-125 sm:scale-[1.4] rotate-12 group-hover/card:scale-[1.6] transition-transform duration-700 text-primary">
              <ind.icon size={28} className="sm:w-10 sm:h-10" />
            </div>
            
            <p className="text-[9px] sm:text-[10px] font-mono tracking-widest uppercase text-muted-foreground/60 mb-1.5 sm:mb-2 font-bold">
              {ind.label}
            </p>
            
            <div className="flex items-end gap-1 sm:gap-1.5 mb-2 sm:mb-3">
               <p className="text-base sm:text-lg md:text-xl font-bold text-foreground drop-shadow-sm truncate">{ind.value}</p>
               <span className="text-[8px] sm:text-[9px] text-muted-foreground font-light pb-0.5 sm:pb-1">{ind.symbol}</span>
            </div>
            ...
            {!loading && ind.isLive && (
              <div className="mt-2 opacity-0 group-hover/card:opacity-100 transition-opacity duration-300">
                <span className="text-[7px] font-mono uppercase tracking-wider text-primary/40 bg-primary/5 px-1.5 py-0.5 rounded">
                  {ind.dataSource}
                </span>
              </div>
            )}
