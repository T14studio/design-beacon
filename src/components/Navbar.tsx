      ...
      <nav className="container mx-auto flex items-center justify-between h-24 px-6">
        ...
        <div className="hidden md:flex items-center gap-6">
          <div className="flex flex-col items-end justify-center mr-2">
            <span className="text-[11px] font-mono tracking-[0.2em] uppercase text-foreground/80 font-bold drop-shadow-sm">
              CRECI 12345-J
            </span>
            <span className="text-[9px] font-mono tracking-[0.25em] uppercase text-primary/80 mt-0.5 font-semibold">
              Desde 2015
            </span>
          </div>
          <a
            ...
            className="flex items-center gap-2 text-[10px] font-bold tracking-widest uppercase text-primary border border-primary/20 hover:border-primary/40 px-5 py-3 rounded-full transition-all duration-500 bg-primary/5 btn-shine"
          >
          ...
          <Link
            to="#"
            className="bg-primary text-primary-foreground text-[10px] font-bold tracking-[0.2em] uppercase px-8 py-3.5 rounded-full hover:bg-primary/90 transition-all shadow-lg active:scale-95 btn-shine"
          >
            Área do cliente
          </Link>
        </div>
        ...
        <div className="md:hidden fixed inset-0 bg-background/95 backdrop-blur-lg z-40 flex flex-col items-center justify-center gap-8">
          {navLinks.map((l) => (
            <Link
              ...
              className={cn(
                "text-2xl font-light tracking-[0.3em] uppercase transition-colors relative group",
          ...
          <a
            ...
            rel="noopener noreferrer"
            onClick={() => setOpen(false)}
            className="flex items-center gap-3 text-xl font-bold tracking-widest uppercase text-primary border border-primary/30 px-8 py-4 rounded-full btn-shine"
          >
            <svg viewBox="0 0 32 32" className="w-6 h-6 text-[#25D366] fill-current">
              <path d="M16.004 2.003c-7.724 0-13.996 6.272-13.996 13.997 0 2.467.654 4.873 1.895 6.989L2 30l7.208-1.884A13.94 13.94 0 0 0 16.004 30c7.724 0 13.996-6.272 13.996-13.997S23.728 2.003 16.004 2.003zm0 25.594a11.58 11.58 0 0 1-5.912-1.617l-.424-.252-4.397 1.152 1.174-4.29-.277-.44a11.57 11.57 0 0 1-1.776-6.15c0-6.408 5.213-11.62 11.62-11.62 6.408 0 11.62 5.213 11.62 11.62s-5.212 11.597-11.628 11.597zm6.37-8.697c-.35-.175-2.07-1.02-2.39-1.137-.32-.117-.554-.175-.787.175s-.903 1.137-1.108 1.372-.408.263-.758.088c-.35-.175-1.477-.544-2.813-1.736-1.04-.926-1.742-2.07-1.946-2.42-.204-.35-.022-.54.153-.714.157-.157.35-.408.525-.613.175-.204.233-.35.35-.583.117-.234.058-.438-.03-.613-.087-.175-.787-1.898-1.078-2.598-.284-.683-.573-.59-.787-.601-.204-.01-.438-.012-.672-.012s-.613.088-.934.438c-.32.35-1.224 1.196-1.224 2.918 0 1.722 1.253 3.386 1.428 3.62.175.233 2.466 3.765 5.976 5.28.835.36 1.486.575 1.994.737.838.266 1.601.228 2.204.138.672-.1 2.07-.846 2.362-1.663.292-.817.292-1.518.204-1.663-.088-.146-.32-.233-.672-.408z"/>
            </svg>
            Falar com especialista
          </a>
          <span className="text-[12px] font-mono tracking-widest uppercase text-foreground/80 font-bold drop-shadow-sm mt-6">
            CRECI 12345-J
          </span>
          <span className="text-[10px] font-mono tracking-[0.3em] uppercase text-primary/80 mt-1 font-semibold">
            Desde 2015
          </span>
        </div>
      )}
