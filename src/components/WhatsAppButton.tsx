...
export default function WhatsAppButton() {
  ...
  // Auto-detect property context if none provided (opened via floating button)
  const currentProperty = useMemo(() => {
    const pathParts = location.pathname.split("/");
    if (pathParts[1] === "imoveis" && pathParts[2]) {
      return properties.find(p => p.id === pathParts[2]);
    }
    return null;
  }, [location.pathname]);

  const effectivePropertyContext = chatData.propertyContext || currentProperty;
  ...
  return (
    <>
      ...
      <AxisChat
        ...
        onClose={() => {
          setIsChatOpen(false);
          // Clear event-driven chatData on close so next open via floating button is clean
          setChatData({});
        }}
